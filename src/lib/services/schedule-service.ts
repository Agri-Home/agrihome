...
import { getMockStore, upsertMockSchedule } from "@/lib/services/mock-store";
...
export const listSchedules = async ({
  ...
  if (!env.useMockData && pool) {
    try {
      ...
      return getMockStore().schedules.filter((schedule) =>
        scopeType ? schedule.scopeType === scopeType : true
      ).filter((schedule) => (scopeId ? schedule.scopeId === scopeId : true));
    }
  }

  return getMockStore().schedules
    .filter((schedule) => (scopeType ? schedule.scopeType === scopeType : true))
    .filter((schedule) => (scopeId ? schedule.scopeId === scopeId : true));
};
...
export const upsertSchedule = async (payload: {
  ...
  if (!env.useMockData && pool) {
    ...
    try {
      ...
      return upsertMockSchedule(payload);
    }
  }
